...
namespace Meta
{
    ...
    QVariant VersionList::data(const QModelIndex &index, int role) const
    {
        ...
        case TimeRole:
            return version->time();
        case RequiresRole:
            return QVariant::fromValue(version->depends());
        case SortRole:
    ...
    void VersionList::setVersions(const QVector<VersionPtr> &versions)
    {
        ...
        auto recommendedIt = std::find_if(m_versions.constBegin(), m_versions.constEnd(), [](const VersionPtr &ptr)
                                          { return ptr->type() == "release"; });
    ...
    void VersionList::setupAddedVersion(const int row, const VersionPtr &version)
    {
        ...
        connect(version.get(), &Version::requiresChanged, this, [this, row]()
                { emit dataChanged(index(row), index(row), QVector<int>() << RequiresRole); });
        connect(version.get(), &Version::timeChanged, this, [this, row]()
                { emit dataChanged(index(row), index(row), QVector<int>() << TimeRole << SortRole); });
